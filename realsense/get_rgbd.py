...
parser.add_argument("--preheat_time", type=int, default=10, help="Preheat time(s) in seconds before starting to save frames.")
...
# Create video writer objects to save the RGB and visualized depth frames
color_video_path = os.path.join(dir_name, "rgb_video.mp4")
vis_video_path = os.path.join(dir_name, "vis_video.mp4")
fourcc = cv2.VideoWriter_fourcc(*'mp4v')
color_writer = cv2.VideoWriter(color_video_path, fourcc, fps, (1280, 720))
vis_writer = cv2.VideoWriter(vis_video_path, fourcc, fps, (1280, 720))
...
try:
    ...
    while True:
        ...
        if save_index > 0:
            ...
            # 缓存图片
            depth_images.append(depth_image.astype(np.float32) / 1000.0)
            color_images.append(color_image.copy())
            vis_images.append(depth_colormap.copy())

            # 写入视频帧
            color_writer.write(color_image)
            vis_writer.write(depth_colormap)
    ...
    # 采集完后统一保存图片
    for idx, (d, c, v) in enumerate(zip(depth_images, color_images, vis_images)):
        np.savez_compressed(os.path.join(depth_dir, f"{idx:05d}.npz"), depth=d)
        cv2.imwrite(os.path.join(color_dir, f"{idx:05d}.png"), c)
        cv2.imwrite(os.path.join(vis_dir, f"{idx:05d}.png"), v)
